  ...
  (function(plugin) {
    ...
    // ECMA-5 15.5.4.11
    // For IE
    if (hasBug('STRING_METHODS_WRONGLY_SETS_REGEXP_LAST_INDEX'))
    ...
    if (hasBug('STRING_REPLACE_COERCE_FUNCTION_TO_STRING') ||
        hasBug('STRING_REPLACE_BUGGY_WITH_GLOBAL_FLAG_AND_EMPTY_PATTERN'))
      plugin.replace = (function(__replace) {
        function replace(pattern, replacement) {
          if (typeof replacement !== 'function')
    ...
    // For Chome 1+
    if (hasBug('STRING_LAST_INDEX_OF_BUGGY_WITH_NEGATIVE_POSITION'))
  ...
  (function(plugin) {
    ...
    function define() {
      ...
      if (!hasFeature('ELEMENT_TEXT_CONTENT')) {
        div.innerHTML = '<pre>&lt;p&gt;x&lt;/p&gt;<\/pre>';

        if (hasFeature('ELEMENT_INNER_TEXT') && div.firstChild.innerText === '<p>x<\/p>')
